...
public delegate byte WANHASyncedAction();
public class WANHAMnetObject : MonoBehaviour
{
    ...
    protected SortedDictionary<byte, WANHASyncedAction> methodDictionary;
    //protected LinkedList<byte> newActions;

    // FINAL VERSION
    protected Dictionary<string, int> getDegate;
       

    // FINAL METODIT

    protected void Trigger(string methodName)
    {
    ...
    protected void Initialize()//MnetObject owner)
    {
        ...
        methodDictionary = new SortedDictionary<byte, WANHASyncedAction>();
        ...
        foreach (MethodInfo meth in methods)
        {
            ...
            if (meth.Name.Length >= 6 && meth.Name.Substring(0,6) == "Action")
            {
                ...
                WANHASyncedAction newAct = (WANHASyncedAction)Delegate.CreateDelegate(typeof(SyncedAction), this, meth);
    ...
    */
    ...
    public bool WriteChanges(MnetPacket packet)//Span<byte> currentPacket, out short currentPacketLength, bool snapshotOverride = false)
    {
        ...
        */
        if(packet.SpaceRemaining < Mnet.minimumPacketSpaceNeededForWrite)
        {
            packet.isFull = true;
        }
