...
{
    [SerializeField]
    [Tooltip("The value stored in this variable.")]
    protected T _value; // Current value of the item.
    private T previous; // Previous value is stored so that it can be used in comparisons etc or to extrapolate action.
    ...
    //!! Stored on turhake eikös oo? Hyvin voidaan käyttää previous arvoa jos uudessa on joku ongelma tai olla vaan tekemäti mitään ja server auth korjata?
    ...
    protected void SetReceivedValue(T newValue)
    {
        _value = newValue;
    }
