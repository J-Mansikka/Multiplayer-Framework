...
public class MnetClientWANHA : MonoBehaviour
{
    //[Tooltip("Object Handler which handles spawn/despawn messaging")]
    //public WANHAMnetObjectStateHandler objectHandler;
    ...
    private WANHAConnectionState connectionState;
    ...
    private void Awake()
    {
        ...
        playerBuffer = new MnetPacketBuffer(MnetSettings.clientPacketBufferSize);
    ...
    */
    ...
    public void HandshakeWithHandler()
    {
        ...
        else
        {
            ...
            {
                Encoding.ASCII.GetBytes(Mnet.messageNewConnectionTest.AsSpan(), connectionEstablisherPacket.Span(0, 16));
    ...
    private void RequestConnection()
    {
        ...
        if (socket.Available > 0)
        {
            ...
            string messageReceived = Encoding.ASCII.GetString(connectionEstablisherPacket.Span(0, 16));
            if (messageReceived == Mnet.messageHandshakeResponse)
            {
                ...
                connectionState = WANHAConnectionState.Handshake;
            }
        }
        else
        {
            if (!ConnectionTimeoutCheck())
            {
                Encoding.ASCII.GetBytes(Mnet.messageHandshakeRequest.AsSpan(), connectionEstablisherPacket.Span(0, 16));
                localEP.Address.TryWriteBytes(connectionEstablisherPacket.Span(16, 4), out _);
                BinaryPrimitives.WriteInt32LittleEndian(connectionEstablisherPacket.Span(20, 4), localEP.Port);
                socket.Send(connectionEstablisherPacket.Span());
            }
    ...
    private void Tick()
    {
        ...
        if (connectionState == WANHAConnectionState.Reconcile)
        {
    ...
    */
    private void ReceivePackets()
    {
        ...
        {
            timeoutTimer = 0f;
            socket.Receive(currentPacket.AllBytes());
            ...
            if (packetNumber != nextExpectedPacketNumber)
            {
                ...
                {
                    connectionState = WANHAConnectionState.MissingPackets;
                }
                missedPackets++;
                /*
            ...
            else
            {
                ...
                if(connectionState == WANHAConnectionState.MissingPackets)
                {
                    ...
                    {
                        connectionState = WANHAConnectionState.Reconcile;
                    }
    ...
    private void ClientUpdate()
    {
        switch (connectionState)
        {
            ...
            case WANHAConnectionState.Handshake:
                ...
                break;
            case WANHAConnectionState.ContactServer:
                RequestConnection();
                break;
            default:
                break;
        ...
        foreach (WANHAMnetObject activeObject in objectsBeingSynced)
        {
            ...
            */
            ...
            while (processingObject)
            {
                ...
                if (sizeOfObject <= (Mnet.maxPacketDataSize - activePacket.currentLength))
                {
    ...
    public void Disconnect()
    {
        if(connectionState == WANHAConnectionState.Connected)
        {
            ...
            Encoding.ASCII.GetBytes(Mnet.messageDisconnectByClient.AsSpan(), connectionEstablisherPacket.Span(1, 16));
            for (int i = 0; i < 3; i++)
            {
