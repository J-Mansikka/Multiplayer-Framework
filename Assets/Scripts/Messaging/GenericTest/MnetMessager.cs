...
public abstract class MnetMessager : MonoBehaviour
{
    ...
    public MnetObject[] worldObjects;
    ...
    */
    ...
    {
        MnetObject[] playerObjects = manager.spawner.GetPlayerObjects();
    ...
    */
            ...
            while(incoming.socket.Available > 0)
            {
                ...
                switch (messageType)
                {
                    ...
                    case MessageType.Regular:
                        {
                            if (receivedPacketNumber == incoming.nextIncomingPacketNumber)
                            {
                                ...
                                do
                                {
                                    ...
                                    incoming.nextIncomingPacketNumber++;
                                }
                    ...
                    case MessageType.MissingPacketsRequest:
                        {
                            ...
                            int packetCount = incoming.playerBuffer.packetForWriting[Mnet.headerMessageTypeLength];
                            int readPos = Mnet.headerMessageTypeLength + 1; // Type + number of missed packets (1-255 = 1 byte)
                            int packetNumber = 0;
                            for (int i = 0; i < packetCount; i++)
                            {
                    ...
                    case MessageType.SnapshotIncoming:
                        {
                            ...
                            if (incoming.firstPacketInSnapshot == -1)
                            {
                                ...
                                if (sequenceNumber != 0)
                                {
                                    ...
                                    incoming.nextIncomingPacketNumber = incoming.firstPacketInSnapshot;
                                }
                                else
                                {
                                    // We got the first one in the update, so the wanted packet number is the next one
                                    incoming.nextIncomingPacketNumber = receivedPacketNumber + 1;
                                }
    ...
    public void SendSnapshotRequest(MnetConnection outgoing)
    {
        ...
        outgoing.state = ConnectionState.Desynced;
    }
