 ...
 * 
 */
public class MnetServerWANHA : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        buffer = new MnetPacketBuffer(69);
    ...
    */
    ...
    private void CheckForNewConnections()
    {
        ...
        while (newConnectionSocket.Available != 0)
        {
            ...
            newConnectionPacket.Reset();

            newConnectionSocket.Receive(newConnectionPacket.AllBytes());//newConnectionPacket.Span());
            string messageReceived = newConnectionPacket.ReadMessage();//Encoding.ASCII.GetString(newConnectionPacket.Span(0, 16));            

            // If the incoming message was identified as a new connection request, extract ip and port
            if (messageReceived == Mnet.messageHandshakeRequest)
            {
                // Add return message identifier
                //Encoding.ASCII.GetBytes(Mnet.messageHandshakeResponse.AsSpan(), newConnectionPacket.Span());
                newConnectionPacket.WriteMessage(Mnet.messageHandshakeResponse);
    ...
    private void UpdatePlayerHandlers()
    {
        ...
        {
            if(client.connectionState == WANHAConnectionState.Disconnected)
            {
    ...
    private void CreatePacketFromTick(float time)
    {
        ...
        foreach (MnetClientHandler client in clients.Values)
        {
            ...
            if (client.connectionState == WANHAConnectionState.SyncWorldState && currentTickNumber != worldStateTickVersion)
            {
    ...
    private void LateUpdate()
    {
        ...
        if (serverRunning)
        {
            ...
            if (checkClientsTimer >= Mnet.clientSendRate)
            {
    ...
    private void CreatePacket(float frameTime, bool createFullSnapshot = false)
    {
        ...
        */
        ...
        if (createFullSnapshot)
        {
            ...
            firstPacketInUpdate.Message = MessageType.SnapshotRequest;
        }
        else
        {
            firstPacketInUpdate = currentPacket;
            firstPacketInUpdate.Message = MessageType.Regular;
        }
