...
public class PlayerHandler
{
    ...
    */
        ...
        if (running)
        {
            ...
            if (!getNextMove)
            {
                ...
                else
                {
                    ...
                    //cc.Move(remainingMove);
                    cc.Move(new Vector3(xMov, 0f, zMov) * playerPacketBuffer[currentMove-1].distance.Item * Mnet.tempPlayerSpeed);
    ...
    public ushort AddMovementPacket(byte[] newMove, int newMoveStartPos)//MonniStoredPlayerStep newPacket)//MonniMovementPacket newPacket)
    {
        ...
        byte currentSequence = newMove[Mnet.sequenceSegment + newMoveStartPos];
